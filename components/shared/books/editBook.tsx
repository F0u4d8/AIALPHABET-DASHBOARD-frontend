...
const EditBookForm: React.FC<CreateContentFormProps> = ({ book  }) => {

 const updateBookWithId = updateBook.bind(null, book.id)
  const [errorMessage, formAction] = useActionState(
    updateBookWithId , undefined
   )
  ...
  const [preview, setPreview] = useState<Preview>(book.image);
 ...
 useEffect(() => {
    if (quill) {
      ...
      quill.clipboard.dangerouslyPasteHTML(book.description || '');

    }
  }, [quill, book.description]);
          ...
          {permissions.map((permission) => (
            <div key={permission.id} className="flex items-center gap-2">
              <input
                ...
                defaultChecked={book.permission.includes(permission.id as Permission)}
              />
